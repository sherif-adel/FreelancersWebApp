...
namespace FreeLancer.Services
{
    ...
    UserContract contractUser = new UserContract(){
                ...
                UserID=user.UserID,FirstName=user.FirstName,LastName=user.LastName,Email=user.Email,Password=user.Password,Mobile=user.Mobile,RoleID=user.RoleID,                Comments=CommentsTranslator.ConvertToCommentContract(user.Comments),                Competitors=CompetitorsTranslator.ConvertToCompetitorContract(user.Competitors),                Profiles=ProfilesTranslator.ConvertToProfileContract(user.Profiles),                Projects=ProjectsTranslator.ConvertToProjectContract(user.Projects),                Requests=RequestsTranslator.ConvertToRequestContract(user.Requests),                Role=RolesTranslator.ConvertToRoleContract(user.Role),                SubCategories=SubCategorysTranslator.ConvertToSubCategoryContract(user.SubCategories),                ProjectsNotifications=ProjectsTranslator.ConvertToProjectContract(user.ProjectsNotifications),};
    return contractUser;}
